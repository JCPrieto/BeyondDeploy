...
public class UtilidadesS3Test {
    ...
    private static S3TransferManager configurarTransferManager(PutObjectResponse... responses) {
        ...
        FileUpload[] uploads = Arrays.stream(responses)
                .map(UtilidadesS3Test::crearFileUpload)
    ...
    public void actualizarCarpetaAgregaCarpetasYArchivos() {
        ...
        ListObjectsV2Response listing = ListObjectsV2Response.builder()
                .contents(
                        S3Object.builder().key("folder1/").build(),
                        S3Object.builder().key("file1.txt").build(),
                        S3Object.builder().key("folder1/file2.txt").build())
                .build();
    ...
    public void wrapAmazonExceptionIncluyeStatusYRequestId() throws Exception {
        Exception ase = S3Exception.builder()
                .message("boom")
                .statusCode(403)
                .requestId("req-1")
    ...
    public void wrapAmazonExceptionMarcaClienteAws() throws Exception {
        SdkClientException ace = SdkClientException.create("boom");
    ...
    public void getObjetosPaginaTodosLosResultados() {
        ...
        ListObjectsV2Response resultado = UtilidadesS3.getObjetos(bucketConfig, "prefix/");

        assertEquals(2, resultado.contents().size());
        assertEquals("prefix/a.txt", resultado.contents().get(0).key());
        assertEquals("prefix/b.txt", resultado.contents().get(1).key());
    ...
    public void crearPaginadorVersionesPaginaBajoDemanda() {
        ...
        ListObjectVersionsResponse page1 = ListObjectVersionsResponse.builder()
                .isTruncated(true)
                .nextKeyMarker("file.txt")
                .nextVersionIdMarker("v1")
                .versions(ObjectVersion.builder().versionId("v1").lastModified(Instant.now()).build())
                .build();

        ListObjectVersionsResponse page2 = ListObjectVersionsResponse.builder()
                .isTruncated(false)
                .versions(ObjectVersion.builder().versionId("v2").lastModified(Instant.now()).build())
                .build();

        when(s3.listObjectVersions(any(ListObjectVersionsRequest.class))).thenReturn(page1, page2);
        ...
        verify(s3, times(2)).listObjectVersions(any(ListObjectVersionsRequest.class));
    }
    ...
    public void getObjectVersionDescargaArchivoEnDirectorioSeleccionado() throws Exception {
        ...
        ResponseInputStream<GetObjectResponse> s3Object = new ResponseInputStream<>(
                ...
                new ByteArrayInputStream("version".getBytes(StandardCharsets.UTF_8)));
        when(s3.getObject(any(GetObjectRequest.class))).thenReturn(s3Object);
    ...
    public void uploadFileSubeArchivoYAplicaPermisos() throws Exception {
        ...
        S3TransferManager transferManager = configurarTransferManager(PutObjectResponse.builder().build());
        ...
        verify(transferManager).uploadFile(argThat((UploadFileRequest request) ->
                !request.transferListeners().isEmpty()));
        verify(s3).putObjectAcl(any(PutObjectAclRequest.class));
    }
